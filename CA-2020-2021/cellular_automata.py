...
class CA(simcx.Simulator):
    ...
    def __init__(self, rule=30, width=50):
        ...
        # The automata implementation will go here
        self.size = width
        self.rule = rule
        self.values = np.zeros(self.size)
        self._next_values = np.zeros(self.size)
        self.cur_step = 0
        self._neighbourhood = (-1, 0, 1)
        self._neighbours = np.zeros(8, dtype=np.uint8)
        self._rule_template = np.unpackbits(np.array([rule], dtype=np.uint8))[::-1]
...
if __name__ == '__main__':
    display = simcx.Display(interval=0.05, visible=False)
